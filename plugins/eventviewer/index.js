...
class Debugger {
    constructor() {
        this.changedSwitches = new Set();
        this.changedSelfSwitches = new Set();
        this.changedVariables = new Set();
        this.switchLogs = [];
        this.selfSwitchLogs = [];
        this.variableLogs = [];
        this.switchDiff = {};
    ...
    restore(capture) {
        ...
        for (const selfSwitchId of this.changedSelfSwitches) {
            $gameSelfSwitches.setValue(selfSwitchId.split(",").map((x) => parseInt(x)), this.selfSwitchDiff[selfSwitchId].old);
        }
        ...
        if (capture) {
            ...
            for (const selfSwitchId of capture.changed.selfSwitches) {
                $gameSelfSwitches.setValue(selfSwitchId.split(",").map((x) => parseInt(x)), capture.diff.selfSwitches[selfSwitchId].new);
            }
...
function parseCommand412(command) {
    // Actually, Do nothing
    return `@412`;
}
function parseCommand404(command) {
    // Actually, Do nothing
    return `@404`;
}
...
function parseCommand301(command) {
    const troopId = (() => {
...
function parseCommand601(command) {
    return `if (battle.win) {`;
}
function parseCommand602(command) {
    return `if (battle.escape) {`;
}
function parseCommand603(command) {
...
function parseCommand111(command) {
    ...
    switch (type) {
        ...
        case 1:
            ...
            const operand2 = command.parameters[2] == 0
